...
Both `llm` and `planner-worker` modes now emit per-run JSONL logs with consistent
event schema.
...
If LangFuse keys are provided, callback metadata is attached to graph invokes.
If unavailable or misconfigured, execution continues with local logs only.
